...
const tileViewportStyle = ({
  ...
  if (tileViewportStyle) return tileViewportStyle();
...
const renderCheckButton = (
  { index, item, isSelectable, onSelectImage }: Props,
  hover: boolean
): ReactNode => {
  return (
    <CheckButton
...
export const GalleryImage: React.FC<Props> = (np: Props) => {
  ...
  const tags =
    typeof props.item.tags === 'undefined' ? (
      <noscript />
    ) : (
  ...
  const customOverlay = !overlayNode ? (
    <noscript />
  ) : (
    <GalleryImageOverlay hover={hover} overlay={overlayNode} />
  );
  ...
  const thumbnailProps = {
    ...
    title:
      typeof props.item.caption === 'string' ? props.item.caption : undefined,
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          zIndex: 1,
          background:
            hover && !props.item.isSelected && props.isSelectable
